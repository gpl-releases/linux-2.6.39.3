 ...
 * - Kernel warning /complains of a too big stack size in the ip6t_webstr_match() function fixed
...
static int mangle_http_header(const struct sk_buff *skb, int flags)
{
    ...
    struct tcphdr *tcph = (void *)iph + sizeof(struct ipv6hdr);
...
static bool ip6t_webstr_match(const struct sk_buff *skb,  struct xt_action_param *par)
{
	const struct ip6t_webstr_info *info = par->matchinfo;
	...
	DEBUGP( PRINTK_LEVEL "\n************************************************\n"
		"%s: type=%s\n", __FUNCTION__, (info->type == IP6T_WEBSTR_URL) 
		? "IP6T_WEBSTR_URL"  : (info->type == IP6T_WEBSTR_HOST) 
		? "IP6T_WEBSTR_HOST" : "IP6T_WEBSTR_CONTENT" );
	...
	switch(info->type)
	{
       ...
       adds HTTP__HOST when HTTP_URL is specified */
		...
		if (opt & (BLK6_JAVA | BLK6_ACTIVE | BLK6_PROXY))
		    flags |= HTTP_URL;
		if (opt & BLK6_PROXY)
		    flags |= HTTP_HOST;
		if (opt & BLK6_COOKIE)
	...
	if (info->type == IP6T_WEBSTR_HOST || info->type == IP6T_WEBSTR_URL) {
	    int nlen = 0, hlen = 0;
	    char needle[WORDSIZE], *haystack = NULL;
	    char *next;

	    if (info->type == IP6T_WEBSTR_HOST) {
...
static struct xt_match webstr6_match __read_mostly = {
	.name		= "webstr",
	.family		= NFPROTO_IPV6,
	.match		= ip6t_webstr_match,
	.matchsize      = sizeof(struct ip6t_webstr_info),
	.me		= THIS_MODULE,
};


static int __init ip6t_webstr_init(void)
{
	return xt_register_match(&webstr6_match);
}

static void __exit ip6t_webstr_fini(void)
{
	xt_unregister_match(&webstr6_match);
}

module_init(ip6t_webstr_init);
module_exit(ip6t_webstr_fini);
