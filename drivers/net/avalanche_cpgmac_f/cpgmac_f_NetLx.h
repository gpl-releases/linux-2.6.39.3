...
typedef struct _CpmacNetDevice {
    ...
    CpmacChInfo infraTxChInfo;
    CpmacChInfo epRxChInfo;
#endif
    ...
    Uint32 Clear_EOI;
    Uint32 unknownProtPkts;                 /**< Unknown protocol packets received */
    /* TxRxParam struct added */
    RxTxParams napiRxTx;
    
#ifdef CPMAC_POLL_MODE
    struct timer_list cpmac_txpoll_timer;   /**< Timer used for polling tc complete */
    struct timer_list cpmac_rxpoll_timer;   /**< Timer for receive operation */
#endif

#define CPGMAC_MAX_LRO_ENTRIES  8

	struct net_lro_mgr lro_mgr;
	struct net_lro_desc lro_arr[CPGMAC_MAX_LRO_ENTRIES];
...
#ifdef CONFIG_ARM_AVALANCHE_PPD
struct EPI {
    unsigned int word0; /* Word 0 contains priority and drop precedence, Ingress peripheral and EPI flags. */
    unsigned int word1; /* Word 1 contains PTID and Session information */
};
#endif
